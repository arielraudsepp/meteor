defmodule MeteorWeb.DiaryEntryController do
  ...
  def index(conn, _params) do
    diary_entries = DiaryEntries.list_diary_entries()
    render(conn, :index, diary_entries: diary_entries)
  end

  def create(conn, %{"diary_entry" => diary_entry_params}) do
    with {:ok, %DiaryEntry{} = diary_entry} <- DiaryEntries.create_diary_entry(diary_entry_params) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"id" => id}) do
    diary_entry = DiaryEntries.get_diary_entry!(id)
    render(conn, :show, diary_entry: diary_entry)
  end

  def show_by_entry_date(conn, %{"entry_date" => entry_date}) do
    diary_entry = DiaryEntries.get_diary_entry_by_date!(entry_date)
    render(conn, :show, diary_entry: diary_entry)
  end
  ...
  def update(conn, %{"id" => id, "diary_entry" => diary_entry_params}) do
    ...
    with {:ok, %DiaryEntry{} = diary_entry} <- DiaryEntries.update_diary_entry(diary_entry, diary_entry_params) do
      render(conn, :show, diary_entry: diary_entry)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, %DiaryEntry{}} <- DiaryEntries.delete_diary_entry(diary_entry) do
