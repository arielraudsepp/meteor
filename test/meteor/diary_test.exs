defmodule Meteor.DiaryTest do
  ...
  describe "diary_entries" do
    ...
    test "list_diary_entries/0 returns all diary_entries" do
      ...
      assert Diary.list_diary_entries() == [diary_entry]
    end

    test "get_diary_entry!/1 returns the diary_entry with given id" do
      diary_entry = diary_entry_fixture()
      assert Diary.get_diary_entry!(diary_entry.id) == diary_entry
    end
    ...
    test "create_diary_entry/1 with valid data creates a diary_entry" do
      ...
      assert {:ok, %DiaryEntry{} = diary_entry} = Diary.create_diary_entry(valid_attrs)
    ...
    test "create_diary_entry/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Diary.create_diary_entry(@invalid_attrs)
    end
    ...
    test "update_diary_entry/2 with valid data updates the diary_entry" do
      ...
      assert {:ok, %DiaryEntry{} = diary_entry} = Diary.update_diary_entry(diary_entry, update_attrs)
    ...
    test "update_diary_entry/2 with invalid data returns error changeset" do
      diary_entry = diary_entry_fixture()
      assert {:error, %Ecto.Changeset{}} = Diary.update_diary_entry(diary_entry, @invalid_attrs)
      assert diary_entry == Diary.get_diary_entry!(diary_entry.id)
    end

    test "delete_diary_entry/1 deletes the diary_entry" do
      diary_entry = diary_entry_fixture()
      assert {:ok, %DiaryEntry{}} = Diary.delete_diary_entry(diary_entry)
      assert_raise Ecto.NoResultsError, fn -> Diary.get_diary_entry!(diary_entry.id) end
    end

    test "change_diary_entry/1 returns a diary_entry changeset" do
      diary_entry = diary_entry_fixture()
      assert %Ecto.Changeset{} = Diary.change_diary_entry(diary_entry)
    end
  end
end
